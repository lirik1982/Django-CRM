...
def register_user(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            login(request, user)
            messages.success(request, "Yoh have successfully Registered!")
            return redirect('home')
    else:
        form = SignUpForm()
...
def custumer_record(request, pk):
    if request.user.is_authenticated:
        ...
        messages.success(request, "Yoh must be logged in to view records!")
        return redirect('home')
...
def delete_record(request, pk):
    if request.user.is_authenticated:
        ...
        messages.success(request, "Yoh must be logged in to do that!")
        return redirect('home')
...
def update_record(request, pk):
    if request.user.is_authenticated:
        ...
        messages.success(request, "Must been logged in...")
        return redirect('home')
